...
const PostBranch = ()=>{
    ...
    const DeleteBranch = async(branch_id) =>{
        try {
          const query = fetch(`http://localhost:5000/api/branch/${branch_id}`,{
              method : 'DELETE'
          });
          console.log(query);
    ...
    const PostBranch = async()=> {
      try {
        ...
        const query = fetch('http://localhost:5000/api/branch',{
          method : 'POST',
    ...
    const GetBranches = async()=> {
      try {
        const query = await fetch('http://localhost:5000/api/branch');
